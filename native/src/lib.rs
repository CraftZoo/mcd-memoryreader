...
use neon::task::Task;
use neon::types::{JsFunction, JsUndefined, JsValue, JsString};
...
fn event_thread(shutdown_rx: mpsc::Receiver<()>, process: reader::Process) -> mpsc::Receiver<Event> {
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            let value = process.read_address("0x0001212323");
            ...
            tx.send(Event::Mem { value }).expect("Send failed");
    ...
    fn complete(
        ...
        match event {
            Event::Mem { value } => {
                ...
                let event_count = cx.number(value as f64);
...
// to JS.
declare_types! {
    pub class JsMemoryEmitter for MemoryEmitter {
        // Called by the `JsEventEmitter` constructor
        init(mut cx) {
            let (shutdown, shutdown_rx) = mpsc::channel();
            let process = reader::Process::new(&cx.argument::<JsString>(0)?.value()[..]);

            // Start work in a separate thread
            let rx = event_thread(shutdown_rx, process);
...
register_module!(mut m, {
    ...
    m.export_class::<JsMemoryEmitter>("MemoryEmitter")?;
